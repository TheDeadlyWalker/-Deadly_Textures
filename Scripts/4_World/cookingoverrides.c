modded class Cooking
{
    ...
    static const int   LIQUID_TRIP_TEA        = 619;  
    ...
    static const string CLASS_TRIP_MUSHROOM        = "Deadly_Space_Shroom";  
    ...
    static const int RECIPE_TRIP         = 5; // -> 619
    ...
    override int CookWithEquipment(ItemBase cooking_equipment, float cooking_time_coef = 1)
    {
        ...
        if (progress >= TEA_COOK_THRESHOLD)
        {
            ...
            if (consumed && pot.GetQuantity() > 0 && pot.GetLiquidType() == LIQUID_WATER)
            {
                ...
                else if (recipe == RECIPE_TRIP)
                    target_liquid = LIQUID_TRIP_TEA;
                else
    ...
    protected int GetAvailableRecipe(EntityAI pot)
    {
        ...
        // Healing -> Energy -> Antibiotics -> Trip -> Death. Adjust to taste.
        ...
        if (HasMushroomClassInPot(pot, CLASS_TRIP_MUSHROOM))
            return RECIPE_TRIP; // NEW
    ...
    protected bool HasMushroomForRecipe(EntityAI pot, int recipe)
    {
        ...
        if (recipe == RECIPE_TRIP)
            return HasMushroomClassInPot(pot, CLASS_TRIP_MUSHROOM); 
